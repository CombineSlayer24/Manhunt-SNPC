...
hook.Add( "PreDrawHalos", "AddShotg_Ammo_Halo", function()
	halo.Add( ents.FindByClass( "sent_vj_mh_shotgun_ammo_pickup*" ), color_org, 8, 8, 5 )
end )
...
function ENT:SpawnFunction(ply, tr)
        ...
        local ent = ents.Create("sent_vj_mh_shotgun_ammo_pickup")
...
if CLIENT then -- Spinning code
		...
		for i = 1, 180 do
			...
			local Pos = MyPos + Forward * i * 0.1
                ...
                self:DrawModel()
                self.Spin = self.Spin + RealFrameTime()*-48
                if self.Spin >= 360 then
                        self.Spin = -360
                end
                self:SetAngles(Angle(36,self.Spin,0))
        end
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:Use(activator, caller) -- Gives ammo when pressing the "Use" key
	if activator:IsPlayer() then
                local ammo_random = math.random(1,10)
